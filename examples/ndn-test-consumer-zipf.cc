...
public:
  ...
  void
  printTime(std::ostream& os, double beginRealTime);
...
Tester::printStats(std::ostream& os, Time nextPrintTime, double beginRealTime)
{
  ...
  for (NodeList::Iterator node = NodeList::Begin(); node != NodeList::End(); node++) {
    ...
    uint32_t pitSize = (*node)->GetObject<ndn::L3Protocol>()->GetObject<ndn::Pit>()->GetSize();
    if (pitSize != 0)
      pitCount += pitSize;
  ...
  Simulator::Schedule(nextPrintTime, &Tester::printStats, this, boost::ref(os), nextPrintTime,
                      beginRealTime);
}

void
Tester::printTime(std::ostream& os, double beginRealTime)
{
  struct ::timeval t;
  gettimeofday(&t, NULL);
  double endRealTime = t.tv_sec + (0.000001 * (unsigned)t.tv_usec);

  os << "The simulation lasted " << endRealTime - beginRealTime << " seconds\n";
}
...
Tester::run(int argc, char* argv[])
{
  ...
  cmd.AddValue("output", "Print Output", m_printOutput);
  cmd.Parse(argc, argv);
  ...
  ndnHelper.SetContentStore ("ns3::ndn::cs::Freshness::Lru", "MaxSize", "100000"); // can set cache size th
  ...
  consumerHelper.SetAttribute ("NumberOfContents", StringValue ("100")); // 100 different contents
  ...
  if (m_printOutput) {
    Simulator::Schedule(Seconds(0), &Tester::printHeader, this, boost::ref(std::cout));
    Simulator::Schedule(m_simulationTime / 200, &Tester::printStats, this, boost::ref(std::cout),
                      m_simulationTime / 200, beginRealTime);
  }
  else {
    Simulator::Schedule(Seconds(29.99), &Tester::printTime, this, boost::ref(std::cout), beginRealTime);
  }
